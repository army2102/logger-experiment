...
export class Logger {
  private level: LogLevel // Log Level => Log only this level or above
  private preHooks: ((log: LogMessage) => void)[] // PreHook => Add or filter data
  private format: (log: LogMessage) => string // Formatter (default) => Format message into a shape that compatible with the transport
  private transports: ((log: LogMessage) => void)[] // PostHook => where to send the log to (default is a terminal)
